...
func StorageUpdate(logger *zap.Logger, db *sql.DB, caller string, updates []*StorageKeyUpdate) ([]*StorageKey, Error_Code, error) {
	...
	err := crdb.ExecuteTx(context.Background(), db, nil, func(tx *sql.Tx) error {
		for i, update := range updates {
			...
			if update.Key.Bucket == "" || update.Key.Collection == "" || update.Key.Record == "" {
				return errors.New(fmt.Sprintf("Invalid update index %v: Invalid values for bucket, collection, or record", i))
			...
			if update.PermissionRead < 0 || update.PermissionRead > 2 {
				return errors.New(fmt.Sprintf("Invalid update index %v: Invalid read permission", i))
			...
			if update.PermissionWrite < 0 || update.PermissionWrite > 1 {
				return errors.New(fmt.Sprintf("Invalid update index %v: Invalid write permission", i))
			...
			if update.Key.UserId != "" {
				if caller != "" && caller != update.Key.UserId {
					...
					return errors.New(fmt.Sprintf("Invalid update index %v: A client can only write their own records", i))
				}
			} else if caller != "" {
				// If the caller is a client, do not allow them to write global data.
				// return nil, BAD_INPUT, errors.New(fmt.Sprintf("Invalid update index %v: A client cannot write global records", i))
				return errors.New(fmt.Sprintf("Invalid update index %v: A client cannot write global records", i))
			}
	...
	WHERE bucket = $1 AND collection = $2 AND user_id = $3 AND record = $4 AND deleted_at = 0`
			...
			if err != nil && err != sql.ErrNoRows {
				...
				return errors.New("Could not update storage")
			}
			...
			if update.Key.Version != "" && ((update.Key.Version == "*" && version != "") || (update.Key.Version != "*" && update.Key.Version != version)) {
				...
				return errors.New(fmt.Sprintf("Storage update index %v rejected: not found, version check failed, or permission denied", i))
			}
			...
			if err != nil {
				...
				return errors.New(fmt.Sprintf("Storage update index %v rejected: %v", i, err.Error()))
			}
...
func StorageRemove(logger *zap.Logger, db *sql.DB, caller string, keys []*StorageKey) (Error_Code, error) {
	...
	if err == nil {
		logger.Info("StorageRemove : Transaction Successfully Executed")
	} else {
		logger.Error("StorageRemove : Could not remove storage, commit error", zap.Error(err))
		return RUNTIME_EXCEPTION, errors.New("Could not remove storage")
	}
